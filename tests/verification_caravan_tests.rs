...
fn verification_skips_caravan_files() {
    ...
    fs::write(src_caravan.join("state.json"), r#"{"mode":"staging","updated":1}"#)
        .expect("create src state");
    // Destination state file (copied earlier, might be older version)
    fs::write(dst_caravan.join("state.json"), r#"{"mode":"staging","updated":0}"#)
    ...
    assert!(report.missing_files.is_empty(), "No files should be missing");
    assert!(report.mismatched_files.is_empty(), "No files should be mismatched (including .caravan)");
}
