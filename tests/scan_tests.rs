...
fn scan_excludes_caravan_directory_at_root() {
    ...
    assert!(!scanned_paths.iter().any(|p| p.contains(".caravan")),
            "Scan should not include .caravan files: {:?}", scanned_paths);
}
...
fn scan_includes_other_hidden_directories() {
    ...
    assert_eq!(scanned.len(), 3, "Should scan 3 files (.git/config, .hidden, visible.txt)");
