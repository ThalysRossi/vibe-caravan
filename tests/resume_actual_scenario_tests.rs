...
fn user_scenario_rerun_transfer_skips_already_copied_batches() {
    ...
    let copy_completed_count = loaded_state
        .batches
        .iter()
        .filter(|b| b.phase == BatchPhase::CopyCompleted)
        .count();
    assert_eq!(
        copy_completed_count, 3,
        "Should have 3 CopyCompleted batches"
    );

    let planned_count = loaded_state
        .batches
        .iter()
    ...
    let verified_count = loaded_state
        .batches
        .iter()
    ...
    for (i, batch) in loaded_state.batches.iter().enumerate() {
        let should_skip =
            batch.phase == BatchPhase::CopyCompleted || batch.phase == BatchPhase::VerifyCompleted;

        if i < 3 {
            assert!(
                should_skip,
                "Batch {} (CopyCompleted) should be skipped",
                batch.batch_id
            );
        } else {
            assert!(
                !should_skip,
                "Batch {} (Planned) should NOT be skipped",
                batch.batch_id
            );
        }
    }
