...
fn state_file_saved_in_both_locations() {
    ...
    assert!(primary_state_dir.exists(), "Primary .caravan directory should exist in source");
    ...
    assert!(!primary_state_files.is_empty(), "Primary state file should exist in source/.caravan/");
    
    // Check secondary location (current directory)
    let secondary_state_dir = tmp.path().join(".caravan");
    assert!(secondary_state_dir.exists(), "Secondary .caravan directory should exist in current dir");
    
    let secondary_state_file = secondary_state_dir.join("state.json");
    assert!(secondary_state_file.exists(), "Secondary state.json should exist in current/.caravan/");
    ...
    for state_file in primary_state_files {
        ...
        let _: serde_json::Value = serde_json::from_str(&content).expect("parse primary state JSON");
    }
    
    let secondary_content = fs::read_to_string(&secondary_state_file).expect("read secondary state file");
    let _: serde_json::Value = serde_json::from_str(&secondary_content).expect("parse secondary state JSON");
}
...
fn status_command_works_with_legacy_state_file() {
    ...
    fs::write(legacy_state_dir.join("state.json"), legacy_state.to_string())
    ...
    assert!(output.status.success(), "status should succeed with legacy state file");
...
fn resume_command_works_with_primary_state_file() {
    ...
    fs::write(primary_state_dir.join(&state_filename), primary_state.to_string())
    ...
    if let Ok(output) = output {
        ...
        assert!(!stderr.contains("No such file or directory") || 
                ...
                "Should not crash with file not found, got: {}", stderr);
    }
}