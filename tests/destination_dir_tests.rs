...
fn destination_subdirectory_fails_when_parent_missing() {
    ...
    assert!(
        !dest_dir.exists(),
        "Destination subdirectory should not be created when parent missing"
    );
}
...
fn capacity_error_message_improved_for_missing_directory() {
    ...
    assert!(
        report.is_ok(),
        "check_capacity should create directory and succeed"
    );
    ...
    assert!(
        report.is_err(),
        "check_capacity should fail for missing parent directory"
    );
