...
pub trait PromptBackend {
    ...
    fn confirm_conflict_skip(&self, batch_id: &str, report: &ConflictReport) -> Result<bool, CaravanError> {
        use std::io::{self, Write};
        ...
        if !report.size_mismatches.is_empty() {
            ...
            for (file, source_size, dest_size) in report.size_mismatches.iter().take(2) {
                println!("     - {} (source: {}, destination: {})", 
                    file.display(), format_bytes(*source_size), format_bytes(*dest_size));
            }
        ...
        print!("\nSkip batch '{}' to avoid overwriting files? (y/n): ", batch_id);
        io::stdout().flush().map_err(|e| 
            CaravanError::Io(format!("failed to flush stdout: {}", e))
        )?;
